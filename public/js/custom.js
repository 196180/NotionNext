window.onload = function() {
  ...
  function insertAIDiv(selector) {
    ...
    if (!targetElement) {
      console.warn('TianliGPT：找不到目标元素。');
      return;
    }
  ...
  var tianliGPT = {
    ...
    fetchTianliGPT: async function(content) {
      ...
      const fetchUrl = 'https://notion.xjfkkk.nyc.mn/api/summary/?token=notionai'; // 确保token和值一致

      try {
        console.log('Fetching AI Summary for Content:', content);
        ...
        if (error.name === "AbortError") {
          ...
          console.error('请求AI摘要发生错误:', error);
        }
        completeGenerate = true;
        return "";
      }
  ...
  function runTianliGPT() {
    ...
    tianliGPT.fetchTianliGPT(content).then(summary => {
      ...
      console.error('请求AI摘要发生错误: ', error);
      const aiExplanationDiv = document.querySelector('.tianliGPT-explanation');
      aiExplanationDiv.innerHTML = "请求AI摘要出错，请稍后重试。";
    });
  }
